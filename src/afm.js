...
export default function afm (arg = '', klass = 'extension', converter = x => x) {
  ...
  for (const ext of exts) {
    ...
    result = result.replace(parts.join('\n'), `${prefix}<div class="${klass} ${type.toLowerCase().replace(/\s/g, '')}"><div>${type}</div><div>${converter(body)}</div></div>\n`);
  }
