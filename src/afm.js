...
export function afm (arg = '', klass = 'extension', compiler = (x = '') => x, map = {}) {
  ...
  for (const ext of exts) {
    ...
    result = result.replace(parts.join(eol), `${prefix}<div class="${klass} ${ctype}"><div>${type}</div><div>${compiler(body)}</div></div>${eol}`);
  }
