...
function afm (arg = '', klass = 'extension', compiler = (x = '') => `${x}\n`, map = {}, label = {}) {
  ...
  for (const ext of exts) {
    ...
    result = result.replace(parts.join(eol), `${prefix}<div class="${klass} ${ctype}">${eol}${prefix}<div>${type in label ? label[type] : type}</div>${eol}${prefix}<div>${eol}${prefix}${compiler(body)}${prefix}</div>${eol}${prefix}</div>${eol}`);
  }
